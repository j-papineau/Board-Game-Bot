...
def parse_board():
    ...
    img = cv2.imread("./tests/tmanepicstyle.jpg")
...
def parse_warp():
    cv_img = []
    for img in glob.glob("warped_imgs/*.jpg"):
        n = cv2.imread(img)
        cv_img.append(n)
    ...
    for image in cv_img:
        ...
        if len(corners) > 0:
            ...
            if not [x for x in (topRight, topLeft, bottomRight, bottomLeft) if x is None]:
                ...
                cv2.imwrite(f'warped{counter}.jpg', warped_img)
                counter += 1
...
def train_model():
    ...
    results = model.train(data="./yolo-training/data.yaml", epochs=100, imgsz=640, batch=16, name='first_piece_model')
